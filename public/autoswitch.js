...
class AutoSwitch {
    ...
    constructor(config = {}) {
        this.config = {
            ...
            clickConfig: { bubbles: true, cancelable: true, view: window },
            callbacks: {
    ...
    cycle() {
        ...
        this.isRunning = true;
        document.body.classList.add('autoswitch-running');
        this.container.classList.add('untouchable');
     ...
     * Wait for loading to complete before proceeding
     */
    _waitForLoadingComplete(callback) {
        ...
        const checkLoading = () => {
            ...
            if (this._isContentLoading() && waitTime < maxWaitTime) {
                waitTime += checkInterval;
                setTimeout(checkLoading, checkInterval);
            } else {
                callback(); // Loading done or timeout reached
            }
        };
    ...
    abort() {
        ...
        document.body.classList.remove('autoswitch-running');
        
        if (typeof this.container.classList !== 'undefined') {
            this.container.classList.remove('untouchable');
        }
